# -*- coding: utf-8 -*-
"""Extraction HD ESSEC : couverture + 3 pages intérieures emblématiques."""
import sys, pathlib
sys.stdout.reconfigure(encoding='utf-8')
import fitz
from PIL import Image
import io

PDF = pathlib.Path(r'C:\Users\sylva\OneDrive\Bureau\DOCBENCH\ESSEC\2025_ESSEC_RAPPORT_DDRS_ENG.pdf')
OUT = pathlib.Path('presentation/images')
OUT.mkdir(parents=True, exist_ok=True)

DPI       = 300
MAX_W     = 2000
JPG_QUAL  = 92

# (clé image, page_idx 0-based)
TARGETS = [
    ('essec.jpg',         0),   # Couverture
    ('essec_inner1.jpg',  3),   # p.6-7 : Governance & Strategy (Together Institute, 3 priorités / 10 commitments / 8 action plans)
    ('essec_inner2.jpg',  7),   # p.14-15 : Environment Key Figures
    ('essec_inner3.jpg',  10),  # p.20-21 : Milestones timeline 1929-2024
]

def extract_page_hd(pdf_path, out_jpg, page_idx):
    doc = fitz.open(str(pdf_path))
    if page_idx >= len(doc):
        page_idx = max(0, len(doc) - 1)
    p = doc[page_idx]
    mat = fitz.Matrix(DPI/72, DPI/72)
    pix = p.get_pixmap(matrix=mat, alpha=False)
    img_bytes = pix.tobytes('png')
    im = Image.open(io.BytesIO(img_bytes)).convert('RGB')
    w, h = im.size
    if w > MAX_W:
        ratio = MAX_W / w
        im = im.resize((MAX_W, int(h * ratio)), Image.LANCZOS)
    im.save(str(out_jpg), 'JPEG', quality=JPG_QUAL, optimize=True, progressive=True)
    doc.close()
    return im.size, out_jpg.stat().st_size // 1024

print(f'Extraction HD ESSEC : DPI={DPI}, max-width={MAX_W}px, JPG q={JPG_QUAL}\n')
for fname, page_idx in TARGETS:
    out = OUT / fname
    size, kb = extract_page_hd(PDF, out, page_idx)
    print(f'  {fname:25s}  p.{page_idx+1:<3}  {size[0]}x{size[1]}  {kb} KB')
